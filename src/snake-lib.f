...
c      write(6,*)'calling itx ', menu(i)
c      write(6,*)'params=',x1+dz,y1+dz/2.
...
c           write(6,*)'call itx 1',hn,ynmin,string
...
c           write(6,*)'call itx 2',xnmin,vn,string
...
cmkj
      ...
      real xsmall,ysmall,zsmall
      integer ixsmall,iysmall,izsmall
      real c00,c10,c01,c11,c0,c1,bfield_int(3)
cmkj
c	write(6,*)'in field'
c
c
...
705     continue
c mkj2
        ixsmall=int((x-xmin)/xstep)+1
        iysmall=int((y-ymin)/ystep)+1
        izsmall=int((z-zmin)/zstep)+1
        xsmall=xmin+(ixsmall-1)*xstep
        ysmall=ymin+(iysmall-1)*ystep
        zsmall=zmin+(izsmall-1)*zstep
     ...
     > izsmall .lt. 1) then
        ...
        do i=1,3
        c00=b(ixsmall,iysmall,izsmall,i)*(1-xdiff)
     >+b(ixsmall+1,iysmall,izsmall,i)*xdiff
        c10=b(ixsmall,iysmall+1,izsmall,i)*(1-xdiff)
     >+b(ixsmall+1,iysmall+1,izsmall,i)*xdiff
        c01=b(ixsmall,iysmall,izsmall+1,i)*(1-xdiff)
     >+b(ixsmall+1,iysmall,izsmall+1,i)*xdiff
        c11=b(ixsmall,iysmall+1,izsmall+1,i)*(1-xdiff)
     >+b(ixsmall+1,iysmall+1,izsmall+1,i)*xdiff
        c0=c00*(1-ydiff)+c10*ydiff
        c1=c01*(1-ydiff)+c11*ydiff
        bfield_int(i)=c0*(1-zdiff)+c1*zdiff
        f(i) = bfield_int(i)
        enddo
        endif
...
c         use 11 values of the field:
	  ...
	  eject=eject.or.(fppp.gt.50.)
          if(eject)then
		write(*,'(a,7(a,i3))')'try to use undefined '
...
7       continue
        endif ! if 1 .eq. -1 mkj to skip the old interpolation
        goto 12
c
c
