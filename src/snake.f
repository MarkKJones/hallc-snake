...
c                previous line : end of the /relfra/ package
      common /comlight/cl(3)
        character*40 fidi,fibl,fidit,fide,fimu,fimut,firay
        character*80 line
        character*1 rep,rip
        character rep2*2
c	character*8 tyme
...
c
        ...
        write(6,*)title(indre)
	if(ilive.le.0)then
		write(6,'(''  alive:'',i6,'' vector, region skipped'')')
     s			ilive
		goto 200
	endif
c
...
c                increment the end-plane index and fill the abs.coo.buff.:
                 call unrot
                 write(6,'(''  alive:'',i6,'' vector(s)'')')ilive
1       continue
200     continue
c
...
601     format(/,' p(lot)',
     ...
     @          ' or q(uit)? (def=p)')
        ...
        if(rep.eq.'V'.or.rep.eq.'v')goto900
        if(rep.eq.'T'.or.rep.eq.'t')goto950
...
cmkj
 900  continue
       do i=1,nuve
             if ( vea(i,nuep,9) .le. 1) then
             do j=0,nuep
             write(31,'(i6,1x,i2,15(1x,f12.5))') i,j,(vea(i,j,k),k=1,15)
             enddo
             endif
          enddo
      goto600
cmkj
 950  continue
       do i=1,nuve
          do j=0,nuep
             if (vea(i,j,9) .ne. -1) then
                write(41,'(3(1x,f12.5))') (vea(i,j,k),k=1,3)
               endif
             enddo
          enddo
      goto600
...
c                next line : begening of the /plot/ package
        logical lbox,luser,bare,ltarget
      integer ul
      real thspec
        character unit*3,name*2
        common /cplot/unit(15),name(15)
...
c
        write(6,'(a)')
     >'$ t(arget) l(oops) r(andom) or i(ndividual) (def.=l):'
        read(ul, '(a)')rep
        if(ldisk)write(6,*)rep
      if(rep.eq.' ')rep='l'
        lrand=rep.eq.'R'.or.rep.eq.'r'
        ltarget=rep.eq.'T'.or.rep.eq.'t'
        if(lrand)then
                 ltarget = .true.
                 write(6,'(a)')'$ number of trajectories :'
                 read(ul,*)istat
                 write(*,*) ' number of traj = ',istat
...
100     format(' x=',f10.3,' y=',f10.3,' z=',f10.3,' tx=',f10.6,
     ...
     @     ' sy=',f8.3,' sz=',f8.3)
      ...
      if (ltarget) then
           ytemp=z*cos(thspec/57.3)
           ztemp=-z*sin(thspec/57.3)
           call inject(x,ytemp
     >,ztemp,cx,cy,cz,psq,px,py,pz)
      else
...
c			cerenkov mirrors?:
c
c      lcer=(cerfiln(iep,indre).ne.'none').or.(cerfiln(iep,indre).ne.'sieve')
      lcer = .false.
...
c		built the diag:
      if(eject)then
c      write(*,'(3a)')'injected outside the free box in :'
c     s			,qdiag(idiag),mdiag
      goto 200
      endif
c
...
c
c mkj add for sieve
      if(cerfiln(iep,indre).eq.'sieve') then
        eject=.true.
         do xh=1,9
               xcent=colli(iep,indre,1)+(xh-1)*colli(iep,indre,3)
            do zh=1,9
               zcent=colli(iep,indre,2)+(zh-1)*colli(iep,indre,4)
c            write(*,*) xcent,x,zcent,z,sqrt((xcent-x)**2+(zcent-z)**2)
c            read(*,*) xcent
            if (sqrt((xcent-x)**2+(zcent-z)**2) .le. 3) then
              eject=.false.
              endif
            enddo
         enddo
...
c mkj
c mkj add for hexagon collimator
      if(cerfiln(iep,indre).eq.'hexcol') then
        eject=.true.
        h_ent=colli(iep,indre,3)
        v_ent=colli(iep,indre,4)
        if ( abs(x) .gt. v_ent) goto 1
        if ( abs(z) .gt. h_ent) goto 1
        if ( abs(x) .gt. (-v_ent/h_ent*abs(z)+1.5*v_ent)) goto 1
        eject=.false.
        goto 1
        endif
c
...
c	iepdead(iv)=iep
c	iredead(iv)=indre
        write(diag(iv),110)iv,iep
       endif
       endif
 110   format(1x,'vector #',i6,'   intercepted by collimator #',i3)
...
c        write(*,*) ' logx,logy,log',logx,logy,logz
         if(eject)then
                 call ucopy(pin,xz,9)
                 goto300
         endif
